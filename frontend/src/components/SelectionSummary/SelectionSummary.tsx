    ...
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex justify-between items-center mb-5">
        ...
        {selectedCount > 0 && (
          <button
            ...
            className="px-4 py-2 bg-red-600 text-white rounded font-medium text-sm hover:bg-red-700 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
          >
            Clear All
          </button>
        )}
      ...
      <ul className="space-y-2 max-h-[240px] overflow-y-auto mb-5">
        {selectedSeats.map((seat) => (
          <li key={seat.id} className="bg-gray-50 rounded p-3 flex items-center justify-between hover:bg-gray-100 transition-colors">
            <div className="flex flex-col gap-1">
              <span className="font-semibold text-gray-900 text-sm">{seat.id}</span>
              <span className="text-xs text-gray-600">Price Tier {seat.priceTier}</span>
            </div>
            <span className="font-bold text-gray-900">${getSeatPrice(seat.priceTier)}</span>
          </li>
        ))}
      </ul>
